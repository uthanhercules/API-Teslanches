...
const listarPedidos = async (req, res) => {
  ...
  try {
    const query = 'select  pedido.id, restaurante_id,consumidor_id,valor_produtos,taxa_entrega,pedido.valor_total, endereco_entrega, nome_usuario from pedido INNER JOIN consumidor on consumidor.id = pedido.consumidor_id where restaurante_id = $1 ORDER BY pedido.id DESC';
